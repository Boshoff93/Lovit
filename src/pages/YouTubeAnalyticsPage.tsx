...
const groupVideosByDay = (videos: YouTubeVideo[]): DayVideos[] => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const sevenDaysAgo = new Date(today);
  sevenDaysAgo.setDate(today.getDate() - 6);

  const byDate = new Map<string, YouTubeVideo[]>();
  videos.forEach(v => {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({
      ...
      isToday,
      videos: (byDate.get(key) || []).sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime()),
    });
  }
  return days;
};
...
const PostCard: React.FC<{ video: YouTubeVideo }> = ({ video }) => (
  <MuiTooltip
    arrow
    placement="top"
        ...
        {days.map((day) => (
          ...
          >
            <Box sx={{
              ...
              <Typography sx={{
                fontSize: '0.7rem',
                color: day.isToday ? 'rgba(255,0,0,0.7)' : 'rgba(255,255,255,0.35)',
              }}>
...
const YouTubeAnalyticsPage: React.FC = () => {
  ...
  const youtubeAccounts = useMemo(
    () => (socialAccountsData?.accounts || []).filter(a => a.platform === 'youtube'),
    [socialAccountsData]
  );
  const activeAccountId = selectedAccountId || youtubeAccounts[0]?.accountId || undefined;
  const activeAccount = youtubeAccounts.find(a => a.accountId === activeAccountId);

  const { data: summary, isLoading: summaryLoading, error: summaryError } = useGetYouTubeSummaryQuery(
    { userId, accountId: activeAccountId },
    { skip: !userId }
  );
  const { data: trendsData, isLoading: trendsLoading } = useGetYouTubeTrendsQuery(
    { userId, accountId: activeAccountId, period: trendPeriod },
    { skip: !userId }
  );
  const { data: accountStats, isLoading: statsLoading } = useGetYouTubeAccountStatsQuery(
    { userId, accountId: activeAccountId },
    { skip: !userId }
  );
  const { data: videosData } = useGetYouTubeVideosQuery(
    { userId, accountId: activeAccountId },
    { skip: !userId }
  );
  ...
  if (summaryError) {
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', minHeight: '80vh', textAlign: 'center', px: 3 }}>
        <BarChartIcon sx={{ fontSize: 64, color: 'rgba(255,255,255,0.2)', mb: 2 }} />
        <Typography variant="h6" sx={{ color: 'rgba(255,255,255,0.5)', mb: 1 }}>YouTube Not Connected</Typography>
        ...
        <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.4)', mt: 1, mb: 3 }}>
          Connect your YouTube account in Settings &gt; Integrations to view analytics.
        </Typography>
        <Button
          variant="contained"
  ...
  const engagementRate = allViews > 0
    ? ((summary?.totals?.likes || 0) + (summary?.totals?.comments || 0)) / allViews * 100
    : 0;
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: { xs: 2, md: 3 } }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0, flex: 1 }}>
          ...
          <Box sx={{ minWidth: 0 }}>
            ...
            <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.6)' }}>
              {accountStats?.channelTitle || 'Performance overview'}
        ...
        {youtubeAccounts.length > 0 && (
          <>
            ...
            >
              <Avatar
                src={activeAccount?.avatarUrl || accountStats?.channelThumbnailUrl}
                sx={{ width: 28, height: 28 }}
              >
                {(activeAccount?.accountName || '?')[0].toUpperCase()}
              </Avatar>
              <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {activeAccount?.accountName || 'Account'}
              </Typography>
      ...
      <Box sx={{
        ...
        <KpiCard
          label="Subscribers"
          value={accountStats?.subscriberCount || 0}
        ...
        <Box sx={{
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={trends}>
              ...
              <Bar dataKey="comments" stackId="engagement" fill="#F59E0B" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Box>
      </Box>
        ...
        <Box sx={{
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={trends}>
              ...
              <Tooltip
                {...chartTooltipStyle}
                formatter={(value: number | undefined) => [value ?? 0, 'Uploads']}
              />
              <Bar dataKey="posts" fill="#FF0000" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Box>
      </Box>
            ...
            <Table>
              ...
              <TableBody>
                {[
                  ...
                  { label: 'Uploads', current: summary.lastWeek.posts, prev: summary.lastMonth.posts / 4 },
                ].map((row) => {
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="right" sx={{ color: 'rgba(255,255,255,0.5)', borderColor: 'rgba(255,255,255,0.08)' }}>Comments</TableCell>
                ...
                {summary.topByViews.map((video, i) => (
                  <TableRow key={video.id}>
                    ...
                    <TableCell align="center" sx={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                      {video.url && (
                        <a href={video.url} target="_blank" rel="noopener noreferrer" style={{ color: '#FF0000' }}>
            ...
            <Table>
              ...
              <TableBody>
                {summary.topByEngagement.map((video, i) => {
                  const totalEng = video.likes + video.comments;
